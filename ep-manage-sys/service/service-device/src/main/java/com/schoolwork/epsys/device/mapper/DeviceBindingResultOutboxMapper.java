...
public interface DeviceBindingResultOutboxMapper extends BaseMapper<DeviceBindingResultOutbox> {

    @Select("""
            SELECT * FROM device_binding_result_outbox
            WHERE (publish_status IN ('PENDING', 'FAILED') AND next_retry_at <= #{now})
               OR (publish_status = 'PUBLISHING' AND updated_at <= #{staleBefore})
            ORDER BY id
            LIMIT #{limit}
            """)
    List<DeviceBindingResultOutbox> findPublishable(@Param("now") Date now,
                                                    @Param("staleBefore") Date staleBefore,
                                                    @Param("limit") int limit);

    @Update("""
            UPDATE device_binding_result_outbox
            SET publish_status = 'PUBLISHING', updated_at = NOW()
            WHERE id = #{id}
              AND ((publish_status IN ('PENDING', 'FAILED') AND next_retry_at <= #{now})
                OR (publish_status = 'PUBLISHING' AND updated_at <= #{staleBefore}))
            """)
    int claimForPublishing(@Param("id") Long id, @Param("now") Date now,
                           @Param("staleBefore") Date staleBefore);
    ...
    @Update("""
            ...
            WHERE id = #{id} AND publish_status = 'PUBLISHING'
            """)
    int markFailed(@Param("id") Long id, @Param("nextRetryAt") Date nextRetryAt,
