...
public interface DeviceBindingOutboxMapper extends BaseMapper<DeviceBindingOutbox> {

    @Select("""
            SELECT * FROM device_binding_outbox
            WHERE (publish_status IN ('PENDING', 'FAILED') AND next_retry_at <= #{now})
               OR (publish_status = 'PUBLISHING' AND updated_at <= #{staleBefore})
            ORDER BY id
            LIMIT #{limit}
            """)
    List<DeviceBindingOutbox> findPublishable(@Param("now") Date now,
                                              @Param("staleBefore") Date staleBefore,
                                              @Param("limit") int limit);

    @Update("""
            UPDATE device_binding_outbox
